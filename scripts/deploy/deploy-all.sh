...
echo ""
echo "1️⃣  正在部署 nginx-ingress-controller..."
kubectl apply -f "$PROJECT_ROOT/k8s/infrastructure/ingress-nginx.yml"
echo "✓ nginx-ingress-controller 部署完成，请等待 Pod 启动（约 1-2 分钟）"

# 2. 等待 nginx 启动
echo ""
echo "2️⃣  等待 nginx-ingress 就绪..."
kubectl wait --namespace ingress-nginx --for=condition=ready pod --selector=app.kubernetes.io/component=controller --timeout=120s || echo "⚠️  nginx 启动超时，请手动检查"
